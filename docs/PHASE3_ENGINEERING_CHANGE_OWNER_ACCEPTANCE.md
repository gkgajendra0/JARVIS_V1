...
- Confirm the normal supervised JARVIS voice runtime is healthy and DBOS uses the accepted production PostgreSQL configuration. Do not point this run at a throwaway SQLite DBOS backend.
...
3. Say an unrelated “yes” and confirm it cannot approve the gate. Say `approve gate_<ID>` using the actual gate ID. Verify one decision is recorded for the exact revision and development starts in an isolated worktree only after that decision.
