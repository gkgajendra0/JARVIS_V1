...
- `ADR-018_SELF_AWARENESS_EVIDENCE_AND_HEALTH_FOUNDATION.md` — owner-approved implementation interlude defining the JARVIS-owned Self Model, deterministic evidence-backed health, OpenTelemetry trace/metric contract, structured redacted logging, separate incident memory, and the no-autonomous-repair boundary;
- `ADR-019_PERSISTENT_CONCURRENT_WORK_ORCHESTRATION.md` — owner-accepted durable WorkItem/WorkStep/WorkDelivery architecture, DBOS/Postgres execution mechanics, single-brain priority, restart-safe unknown-outcome handling, protected payloads, deferred delivery, bounded research and isolated development.
- `ADR-019_SELF_REPAIR_FOUNDATION.md` — owner-approved bounded Self-Repair foundation defining deterministic RepairPolicy authority, durable repair attempts, process-external crash/liveness recovery, verification, fail-closed budgets, and the later review-gated RepairKnowledge/diagnostic curriculum boundary.
