...
def test_owner_only_calibration_reports_stable_positive_baseline() -> None:
    ...
    assert result.status is OwnerCalibrationStatus.BASELINE_CAPTURED
    assert result.owner_reference_count == 65
    assert result.owner_probe_count == 65
    assert result.provisional_positive_floor == pytest.approx(result.owner_scores.p05)
...
def test_owner_only_calibration_reports_wide_positive_baseline_without_inventing_threshold() -> None:
    ...
    assert result.status is OwnerCalibrationStatus.BASELINE_CAPTURED
    assert result.provisional_positive_floor < 0.70
    assert result.owner_scores.median < 0.70
