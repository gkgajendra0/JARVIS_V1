...
def _phase_mask(frame_count: int, frame_seconds: float, phase: LanePhase) -> np.ndarray:
...
def _summarize_phase(
    ...
    *,
    ...
    phase: LanePhase,
    threshold: float,
) -> _PhaseProbabilityStats:
    selected = probabilities[_phase_mask(probabilities.size, frame_seconds, phase)]
    if selected.size == 0:
        raise RuntimeError(f"pVAD produced no probability frames for {phase.name}")
    return _PhaseProbabilityStats(
        frame_count=int(selected.size),
        mean=float(np.mean(selected)),
        median=float(np.median(selected)),
        p95=float(np.percentile(selected, 95)),
        maximum=float(np.max(selected)),
        active_fraction=float(np.mean(selected >= threshold)),
    )
...
def _format_optional(value: float | None, *, suffix: str = "") -> str:
    if value is None:
        return "n/a"
    return f"{value:.3f}{suffix}" if not suffix else f"{value:.0f}{suffix}"


async def run_personalized_vad_benchmark(
    *,
    asset_dir: Path | None,
    ...
    print("JARVIS Step 3 personalized target-speaker VAD benchmark")
    print("------------------------------------------------------")
    ...
    try:
        ...
        consumer_task = asyncio.create_task(
            ...
            name="jarvis-personalized-vad-benchmark-capture",
        )
        ...
        reference_samples, reference_rate = await _capture(
            recorder,
            label="S_OWNER_REFERENCE",
            instructions=(
                "Phone/TV silent. Keep your face visible and speak naturally for the whole "
                "capture. This reference is memory-only and will NOT be enrolled."
            ),
        ...
        candidate = await asyncio.to_thread(FireRedPersonalizedVad, assets)
        model_build_ms = (time.perf_counter() - build_started) * 1000.0
        target_embedding, embedding_ms = await asyncio.to_thread(
            candidate.build_target_embedding,
            reference_samples,
        ...
        print("STEP_3_PERSONALIZED_VAD_BENCHMARK = EVIDENCE_CAPTURE_COMPLETE")
        print(
            "Human review must compare phone rejection, OWNER preservation/overlap, "
            "transition latency, and realtime cost before selecting this candidate."
        )
        return 0
    finally:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark FireRedChat personalized VAD on canonical JARVIS PCM using "
            "a temporary memory-only OWNER reference."
        )
    )
...
def main() -> None:
    ...
    try:
        code = asyncio.run(
            run_personalized_vad_benchmark(
        ...
        FireRedPVadUnavailable,
        PersonalizedVadAssetError,
        ValueError,
        RuntimeError,
    ) as exc:
        print(f"personalized VAD benchmark failed: {exc}")
        code = 2
