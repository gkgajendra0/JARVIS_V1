...
from jarvis.knowledge.web_search import (
    EvidenceSource,
    ResearchMode,
    SearchProviderEvidence,
    utc_now,
)
...
class ExaWebSearchProvider:
    """Search Exa and return source excerpts; never synthesize the final answer."""
    ...
    def search(self, query: str, mode: ResearchMode) -> SearchProviderEvidence:
        ...
        if not isinstance(raw_results, Sequence) or isinstance(
            ...
            return SearchProviderEvidence(sources=())
        ...
        return SearchProviderEvidence(sources=tuple(sources))

    def close(self) -> None:
        # exa-py currently exposes no client-level close requirement for sync usage.
        self._client = None


def build_current_research_service(*, timeout_seconds: float = 30.0):
    from jarvis.knowledge.web_search import CurrentResearchService

    return CurrentResearchService(
