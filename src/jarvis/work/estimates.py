...
def _human_duration(seconds: int) -> str:
    ...
    hours = max(1, round(value / 3600))
    return f"{hours} hour" if hours == 1 else f"{hours} hours"


def _eta_range_text(estimate: WorkEstimate) -> str | None:
    low = estimate.eta_low_seconds
    high = estimate.eta_high_seconds
    if low is None or high is None:
        return None
    if low == high:
        return _human_duration(low)
    return f"{_human_duration(low)} to {_human_duration(high)}"


def owner_work_status_summary(
    work: WorkItem,
    estimate: WorkEstimate,
    *,
    completion_notification_expected: bool,
) -> str:
    """Build deterministic owner-facing status from canonical work truth."""

    if work.state is WorkState.COMPLETED:
        return "The task is complete. No work remains."

    progress_prefix = (
        f"The task is approximately {estimate.progress_percent}% complete."
        if estimate.progress_is_approximate
        else f"The task is {estimate.progress_percent}% complete."
    )
    parts = [progress_prefix, estimate.progress_summary]

    if estimate.blocked_reason:
        parts.append(f"Current blocker: {estimate.blocked_reason}.")

    parts.append(f"Remaining work: {estimate.remaining_summary}")

    eta_range = _eta_range_text(estimate)
    if eta_range is not None:
        parts.append(
            f"Estimated completion: {eta_range}, "
            f"with {estimate.eta_confidence} confidence."
        )
    elif estimate.eta_confidence == "unavailable":
        parts.append(f"ETA unavailable: {estimate.eta_reason}")

    if completion_notification_expected and not work.state.terminal:
        parts.append("I will let you know when it is complete.")
