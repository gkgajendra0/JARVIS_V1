...
# Generic installed-app lifecycle: app names are Windows catalogue data, not code allowlists.
replace_once(
...
class _InjectedBackendCatalog:
    """Safe identity catalogue used only when tests/custom code inject an executor backend."""
...
class CatalogAppLifecycleBackend:
    """Launch one resolved Windows AppsFolder identity and verify with winapp."""
...
class AppLifecycleExecutor:
    ...
    def prepare(self, request: CapabilityRequest) -> PreparedCapability:
        ...
        return PreparedCapability(
            ...
            material_summary=f"Open approved Windows application: {app.display_name}",
...
# Structured app UI becomes generic for already-resolved/running apps. Only the legacy
# nested launch action remains allowlisted; goal plans launch through app.lifecycle.
replace_once(
...
# Operation resolution now follows the semantic registry's substrate priority if more
# than one executor can satisfy an operation.
replace_once(
...
# Live voice exposes one Hands action tool rather than one native tool plus one UI tool.
replace_once(
...
# Replace implementation-shaped voice instructions with the single goal boundary.
regex_once(
...
# PyWinRT is modular: Media.Control uses Windows.Foundation async types at runtime.
replace_once(
    "pyproject.toml",
...
print("goal-oriented Hands architecture refactor applied")
