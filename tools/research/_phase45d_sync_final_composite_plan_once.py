"""One-time helper to synchronize CURRENT_PLAN after final composite freeze."""
...
EXPECTED_HASH = "69666a37d436828b1d65827852f9e43d524253608209275205c41a36f8accadf"


def main() -> None:
    raw = subprocess.check_output(
        ["python", "tools/research/step4_phase45d_final_composite_cases.py"],
        text=True,
    )
    corpus = json.loads(raw)
    if corpus["sha256"] != EXPECTED_HASH:
        raise RuntimeError(
            f"Frozen corpus hash changed: {corpus['sha256']} != {EXPECTED_HASH}"
        )
    shape = (
        corpus["documents"],
        corpus["queries"],
        corpus["release_cases"],
        corpus["abstain_cases"],
    )
    if shape != (55, 255, 90, 165):
        raise RuntimeError(f"Frozen corpus shape changed: {shape}")
    ...
    if old_stage not in text:
        raise RuntimeError("CURRENT_PLAN current-stage marker changed unexpectedly")
    text = text.replace(old_stage, new_stage, 1)

    start = text.index("## Active 4.5D direction")
    end = text.index("## Phase 4.5E — BLOCKED", start)
    replacement = """## Active 4.5D direction — PROVIDER-INDEPENDENT COMPOSITE GATE SELECTED / FRESH ACCEPTANCE NEXT
...
"""
    ...
    old_order = (
        "9. **4.5D — ACTIVE: V3 FAIL_CALIBRATION / RETIRED with validation unexposed; "
        "Gemini 3.8 calibration-only diagnostic next.**"
    )
    new_order = (
    ...
    if old_order not in text:
        raise RuntimeError("CURRENT_PLAN implementation-order marker changed unexpectedly")
